...
For non-Hermes targets, use `scripts/install-target-repair.sh TARGET`. It uses the registered BRECC target root for Claude, Cursor, Antigravity, Codex, Gemini, OpenCode, OpenClaw, CodeBuddy, JoyCode, Kimi, Qwen, and Zed, and installs the same target-neutral helper at `<target-root>/.brecc/repair.sh`. Home and project roots can be supplied with `--home-dir`, `--project-root`, or `--target-root`; the installer refuses to guess unsupported or missing layouts and refuses an unexpected existing helper unless `--replace` is explicit.
